...
#
# Mesmo contrato dos modelos colaborativos:
...
#
# PURO content-based: não usa co-ocorrência em playlists nem qualquer sinal colaborativo — a
# recomendação vem só do CONTEÚDO da faixa (features de áudio, gênero do artista e ano de lançamento).
#
# "Treinar" aqui é engenharia de atributos sobre 'audio_features.csv' + 'generos.json' (padroniza/
# normaliza as 10 features de áudio, vetoriza os gêneros em matriz esparsa, deriva ano/era/país do
# ISRC). Os artefatos vão para saved_models/content_based.joblib e são recarregados nas próximas
# execuções (não retreina se o arquivo existir; apague-o para forçar o retreino).
#
# Limpeza de ruído no treino: o CSV tem ~2,26M faixas, mas ~42% NÃO têm features de áudio (linhas
# vazias) — essas são descartadas (dropna). Faixas sem gênero (~61%) ou sem ISRC (~0,2%) são mantidas:
# ainda têm áudio/ano, e o sinal ausente só contribui zero na soma ponderada (descartá-las mataria a
# cobertura à toa).
#
...
#       representa a playlist pelo PERFIL-CENTROIDE das faixas conhecidas e ranqueia o catálogo por
#       similaridade a esse perfil. Ponte via URI; faixas fora do CSV de áudio ficam de fora.
#
# Dados necessários:
...
#
# O estudo de ablação / feature selection (precisa das playlists em ./archive/data) NÃO roda no import
# nem no treino: é pesquisa, disponível só na execução direta deste arquivo, atrás da flag
# RUN_ABLATION.
#
...
# Pesos da soma ponderada de scores (não precisam somar 1; normalizados internamente). O artista
# domina o sinal de co-ocorrência em playlist; gênero e ano entram pequenos como complemento/
# desempate. Sinais disponíveis: score_funcs.
...
class ContentRecommender:
    ...
    def __init__(self, df, mat_audio, mat_generos, track_map=None, reverse_track_map=None):
        ...
        # Códigos inteiros das features categóricas — usados no perfil-centroide (cada faixa recebe
        # como score a FRAÇÃO das seeds que compartilham sua categoria). Computados aqui (não salvos):
        # dependem só do df carregado.
        ...
        # Ponte para a interface colaborativa (track_encoded <-> uri). Vêm dos dados do
        # data_processing, NÃO do modelo salvo (dependem do N_FILES).
    ...
    def _set_bridge(self, track_map, reverse_track_map):
        ...
        self.cidx_to_enc = np.fromiter((track_map.get(u, -1) for u in self.uris), dtype=np.int64, count=self.N)
    ...
    # --------------------------------------------------------------------- #
    @classmethod
    def load_or_train(cls, interactions_df=None, pid_map=None, track_map=None, reverse_track_map=None, uri_to_name=None):
        # Assinatura idêntica aos colaborativos. interactions_df, pid_map e uri_to_name não são usados
        # (o content treina do CSV); track_map/reverse_track_map só servem à ponte usada pelo
        # recommend() colaborativo.
        if model_exists():
    ...
    def train(cls, track_map=None, reverse_track_map=None):
        ...
        # Leitura em blocos + dropna por bloco (poupa RAM; descarta as ~42% de faixas SEM features de
        # áudio, que não têm sinal de conteúdo).
        usecols = ['uri', 'nome', 'artista', 'isrc'] + AUDIO_FEATURES
        partes = []
        total = 0
        for ch in pd.read_csv(CSV_PATH, usecols=usecols,
                              dtype={'uri': str, 'nome': str, 'artista': str, 'isrc': str},
                              chunksize=CSV_CHUNK):
            total += len(ch)
        ...
        print(f"[Content] Faixas: {len(df):,} com áudio (descartadas {total - len(df):,} sem features, de {total:,})")

        # Ano e Era via ISRC
        anos = df['isrc'].apply(_isrc_para_ano)
        df['ano'] = anos.fillna(anos.median())
        df['era'] = df['ano'].apply(_era_musical)
        print(f"[Content] Ano: {int(df['ano'].min())}–{int(df['ano'].max())} | sem ISRC: {anos.isna().sum():,}")
        ...
        print(f"[Content] Gêneros únicos: {len(mlb.classes_):,} | faixas sem gênero (sinal=0): {sem_genero:,} ({sem_genero/len(df):.0%})")
    ...
    def load(cls, track_map=None, reverse_track_map=None):
        ...
        return cls(payload['df'], payload['mat_audio'], payload['mat_generos'], track_map, reverse_track_map)

    # --------------------------------------------------------------------- #
    # SCORES INDIVIDUAIS (por faixa) — cada um recebe o índice da faixa-alvo e devolve um array (N,)
    # em [0, 1] com a similaridade a todas as faixas.
    # --------------------------------------------------------------------- #
    ...
    # --------------------------------------------------------------------- #
    def recomendar(self, nome_musica: str, pesos: dict = None, top_k: int = 20, excluir_mesmo_artista: bool = False) -> pd.DataFrame:
        ...
        resultado = self.df.loc[top_idx, ['nome', 'artista', 'ano', 'era', 'generos', 'pais_isrc']].copy()
        resultado['score'] = score[top_idx].round(4)
        resultado['ano'] = resultado['ano'].astype(int)
        resultado['generos'] = resultado['generos'].apply(
            lambda x: ', '.join(x[:3]) + ('...' if len(x) > 3 else '') if isinstance(x, list) and x else 'Nenhum')

        row = self.df.loc[idx_alvo]
        gen_alvo = ', '.join(row['generos']) if row['generos'] else 'Desconhecido'
        print(f"\n[Content] Top {top_k} para: '{row['nome']}' - {row['artista']} ({int(row['ano'])}) | Generos: {gen_alvo}")
        return resultado.reset_index(drop=True)

    # --------------------------------------------------------------------- #
    # INFERÊNCIA COLABORATIVA-COMPATÍVEL — mesma assinatura dos colaborativos. PURO content-based: a
    # playlist é o PERFIL-CENTROIDE das faixas conhecidas (perfil de gênero/ano + frequência de
    # artista/país, conforme os pesos) e o catálogo é ranqueado por similaridade a esse perfil. Ponte
    # via URI: faixas fora do CSV de conteúdo não entram no ranking.
    # --------------------------------------------------------------------- #
    @staticmethod
    ...
    def _freq_perfil(self, codes: np.ndarray, n_codes: int, cidx: np.ndarray) -> np.ndarray:
        """Perfil de frequência categórico: score (N,) = fração das seeds que compartilham a
        categoria de cada faixa do catálogo (em [0, 1])."""
    ...
    def _centroid_scores(self, cidx: np.ndarray, pesos: dict) -> np.ndarray:
        ...
        if len(cidx) == 0:
            return score
        # Sinais suportados no perfil-centroide: vetoriais (áudio, gênero), ano (kernel gaussiano em
        # torno do ano médio) e categóricos (artista, país, era) via perfil de frequência. Pesos de
        # sinais não listados são 0.
    ...
    def score(self, pid_encoded, seed_idxs, pesos: dict = None):
        # Vetor de scores de conteúdo (num_tracks,) do catálogo contra o perfil-centroide das seeds,
        # no espaço colaborativo (track_encoded), sem exclusões. Faixas sem features de áudio (fora
        # do CSV) ficam -inf.
        ...
        # Catálogo -> espaço colaborativo (track_encoded). Faixas que o colaborativo não conhece ficam
        # de fora; as não cobertas ficam -inf.
        scores = np.full(self.num_tracks, -np.inf, dtype=np.float64)
        valido = self.cidx_to_enc >= 0
        scores[self.cidx_to_enc[valido]] = scores_cat[valido]
        return scores

    def recommend(self, pid_encoded, seed_idxs, exclude_idxs, top_k=500, pesos: dict = None):
...
# =============================================================================
# PESQUISA — Ground Truth + Feature Selection (ablação). NÃO roda no treino; só na execução direta
# deste arquivo com RUN_ABLATION=True (precisa das playlists em ./archive/data).
# =============================================================================
...
def feature_selection_ablation(rec: ContentRecommender, pasta_json='./archive/data', output_csv='feature_selection.csv'):
    """Estudo de ablação: avalia cada combinação de features contra o ground truth de co-ocorrência.
    Pesado e dependente de ./archive/data."""
    ...
    def get_coocorrencias(uri_alvo, min_cooc=MIN_COOC):
        ...
        if not pls:
            return set()
        return {u for u in uri_series if u != uri_alvo and len(pls & track_playlists.get(u, set())) >= min_cooc}
...
# =============================================================================
# EXECUÇÃO DIRETA — apenas TREINA (mesmo padrão dos colaborativos). Defina RUN_ABLATION=True para
# rodar o estudo de feature selection (lento e dependente de ./archive/data).
# =============================================================================
...
if __name__ == "__main__":
    ...
    r = rec.recomendar("Billie Jean", PESOS_DEFAULT, top_k=15, excluir_mesmo_artista=True)
    if not r.empty:
