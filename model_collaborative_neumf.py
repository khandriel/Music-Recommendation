# =============================================================================
# model_collaborative_neumf.py — Colaborativo puro via rede neural (Neural Collaborative Filtering /
# NeuMF)
#
# Aprende a relação playlist↔música a partir das co-ocorrências (quais músicas aparecem juntas nas
# mesmas playlists), sem usar nenhum metadado textual (nome/artista). Características principais:
...
#   4. ReduceLROnPlateau + EarlyStopping
#   5. Reprodutibilidade controlada por SEED
#
# Só TREINA e expõe a inferência (recommend). A avaliação — mesmo conjunto de teste para todos os
# modelos — vive em compare_models.py.
#
...
#
# Combina duas branches: a GMF (produto elemento a elemento dos embeddings, capta interações
# lineares) e a MLP (concatenação + densas empilhadas, capta padrões não-lineares). As duas saídas
# são concatenadas e passam por uma sigmoid final.
# =============================================================================

def build_model(num_playlists: int, num_tracks: int, embedding_dim: int = EMBEDDING_DIM) -> Model:
    ...
    # Branch 1 — GMF: relações lineares via produto elemento a elemento
    p_emb_gmf = Embedding(num_playlists, embedding_dim, embeddings_regularizer=l2(L2_REG), name="playlist_emb_gmf")(p_input)
    t_emb_gmf = Embedding(num_tracks, embedding_dim, embeddings_regularizer=l2(L2_REG), name="track_emb_gmf")(t_input)
    gmf_out = Multiply()([Flatten()(p_emb_gmf), Flatten()(t_emb_gmf)])

    # Branch 2 — MLP: padrões não-lineares com camadas densas empilhadas
    p_emb_mlp = Embedding(num_playlists, embedding_dim, embeddings_regularizer=l2(L2_REG), name="playlist_emb_mlp")(p_input)
    t_emb_mlp = Embedding(num_tracks, embedding_dim, embeddings_regularizer=l2(L2_REG), name="track_emb_mlp")(t_input)
    mlp_out = Concatenate()([Flatten()(p_emb_mlp), Flatten()(t_emb_mlp)])

    for units in MLP_LAYERS:
        mlp_out = Dense(units, activation='relu', kernel_regularizer=l2(L2_REG))(mlp_out)
...
def build_training_data(interactions_df, num_tracks: int, neg_ratio: int = NEG_RATIO, seed: int = SEED):
    """
    Gera pares positivos e negativos garantindo que os negativos não sejam músicas que a playlist
    realmente contém. Para cada par positivo (playlist, música), sorteia `neg_ratio` músicas que
    estão FORA da playlist, eliminando falsos negativos que confundem o treino.

    O sorteio é vetorizado (numpy) e só as colisões são re-sorteadas, evitando o laço Python por
    linha da versão anterior.
    """
    print(f"[NeuMF] Gerando amostras de treino (negative sampling seguro, neg_ratio={neg_ratio})...")
    ...
    pid_to_tracks = interactions_df.groupby('pid_encoded')['track_encoded'].apply(set).to_dict()

    # Cada positivo gera `neg_ratio` negativos → replica os pids. int32 nos índices e float32 nos
    # rótulos cortam o uso de RAM pela metade ante os defaults int64/float64 do numpy.
    ...
    collision_idx = np.array([i for i, (p, t) in enumerate(zip(neg_pids, neg_tracks)) if t in pid_to_tracks[p]])
    while len(collision_idx) > 0:
        neg_tracks[collision_idx] = rng.integers(0, num_tracks, size=len(collision_idx), dtype=np.int32)
        collision_idx = collision_idx[[neg_tracks[i] in pid_to_tracks[neg_pids[i]] for i in collision_idx]]
...
def train(interactions_df, num_playlists: int, num_tracks: int) -> Model:
    """Treina o NeuMF com callbacks de LR scheduling e early stopping."""
    # Semeia numpy, random e TensorFlow de uma vez — inicialização dos pesos e dropout reprodutíveis
    # entre execuções.
...
# Mesma interface dos demais colaborativos: load_or_train() + recommend().
# =============================================================================
...
class NeuMFRecommender:
    ...
    def score(self, pid_encoded, seed_idxs=None):
        # Vetor de scores NeuMF (num_tracks,) para `pid_encoded` sobre todo o catálogo, sem exclusões.
        # `seed_idxs` é ignorado — o NeuMF representa a playlist pelo embedding aprendido no treino
        # (id da playlist).
        all_track_idxs = np.arange(self.num_tracks)
        return self.model.predict([np.full(self.num_tracks, pid_encoded), all_track_idxs],
                                  batch_size=4096, verbose=0).flatten().astype(np.float64)

    def recommend(self, pid_encoded, seed_idxs, exclude_idxs, top_k=500):
        # Top-k músicas pelo score NeuMF para `pid_encoded`, excluindo `exclude_idxs`. `seed_idxs` é
        # ignorado (ver score()).
...
if __name__ == "__main__":
    ...
    print("[NeuMF] Concluído. Para avaliar/comparar: python compare_models.py")
