...
#
# Combina um colaborativo (NeuMF, ALS ou Item-kNN) com o Content-Based sem retreinar: compõe dois
# modelos já salvos em saved_models/ e expõe o mesmo recommend(pid, seed_idxs, exclude_idxs, top_k)
# dos demais.
#
# Fusão (weighted hybrid):
#   1. Pega de cada modelo o vetor de scores sobre o catálogo (score()).
#   2. Exclui as faixas conhecidas (seeds) antes de normalizar, para o min-max cobrir só as
#      candidatas.
#   3. Normaliza cada vetor para [0,1] por min-max (as escalas diferem: NeuMF é sigmoide, ALS é
#      produto escalar, kNN é soma de similaridades, content é soma ponderada).
#   4. Combina por média ponderada com peso ALPHA no colaborativo:
#          score = alpha * colab_norm + (1-alpha) * content_norm
#      só sobre os modelos presentes na faixa (peso renormalizado por faixa): faixa sem cobertura do
#      content usa só o colaborativo; modelo com score constante (sem poder de discriminação) também
#      se abstém.
#
# COLAB_DEFAULT/ALPHA definem o par e o peso; HybridRecommender.build(data, colab_key=..., alpha=...)
# compõe um par específico.
#
...
COLAB_REGISTRY = {
    "neumf":   ("model_collaborative_neumf",   "NeuMFRecommender",    os.path.join(MODELS_DIR, "collaborative_neumf.keras")),
    "als":     ("model_collaborative_als",     "ALSRecommender",      os.path.join(MODELS_DIR, "collaborative_als.npz")),
    "itemknn": ("model_collaborative_itemknn", "ItemKNNRecommender",  os.path.join(MODELS_DIR, "collaborative_itemknn.npz")),
}
...
def _minmax(scores: np.ndarray):
    # Normaliza as entradas FINITAS para [0,1] e retorna (norm, present): `present` marca onde havia
    # sinal (finito E o modelo discrimina). Entrada -inf (modelo se abstém) vira norm=0 e
    # present=False; score constante (hi<=lo, sem poder de ranquear) também vira abstenção total.
...
class HybridRecommender:
    ...
    def build(cls, data, colab_key: str = COLAB_DEFAULT, alpha: float = ALPHA):
        ...
        print(f"[Híbrido] Compondo {cls_name} + Content (alpha={alpha}: {alpha:.0%} colab / {1 - alpha:.0%} content)...")
    ...
    def recommend(self, pid_encoded, seed_idxs, exclude_idxs, top_k=500):
        ...
        # Média ponderada SÓ sobre os modelos presentes (peso renormalizado por faixa): faixa sem
        # content usa só o colaborativo, sem ser penalizada.
...
if __name__ == "__main__":
    ...
    print(f"[Híbrido] Pronto: {rec.nome}. Para avaliar/comparar: python compare_models.py")
